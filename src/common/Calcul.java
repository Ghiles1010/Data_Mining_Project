...
public class Calcul {
    ...
    //***********************************Symetrie***********************************//
    public static String symetrique(Dataset ds, int X_index){
        int moy = (int) moy(ds, X_index);
        int med = (int) mediane(ds, X_index);
        int mode = (int) mode(ds, X_index);

        if(moy == med && moy == mode){
            return "les données de l'Att: "+X_index+" sont Symetrique";
        }
        if(moy < med && med <mode){
            return "les données de l'Att: "+X_index+" sont Asymetrique Négativement";
        }
        if(moy > med && med > mode){
            return "les données de l'Att: "+X_index+" sont Asymetrique Positivement";
        }
        return "";
    }
